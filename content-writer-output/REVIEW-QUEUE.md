# Coda review contenuti — valore-immobili.it

Ordine consigliato: prima hub/guide sottili ad alto impatto SEO o conversione, poi leaf.

Stato: **1 fatto** · resto in coda. Procediamo **una pagina alla volta**.

| # | URL | Priorità | Motivo | Status |
|---|-----|----------|--------|--------|
| 1 | `/comprare-casa-brianza` | — | Guida lunga + bold + link interni | **Fatto** (2026-08-03) |
| 2 | `/comprare-casa-monza` | Alta | Hub acquisto; ancora relativamente corta | Prossima |
| 3 | `/vendere-casa-monza` | Alta | Hub vendita; intent correlato a valutazione | Coda |
| 4 | `/annunci-casa-monza` | Alta | Supporta cluster acquisto + CTR query | Coda |
| 5 | `/ville-in-vendita-brianza` | Media | Tipologica Brianza, thin | Coda |
| 6 | `/appartamenti-in-vendita-monza` | Media | Tipologica Monza, thin | Coda |
| 7 | `/spese-acquisto-casa` | Media | Intent informativo forte | Coda |
| 8 | `/offerta-acquisto-casa` | Media | Vicino a conversione | Coda |
| 9 | `/mutuo-prima-casa` | Media | Intent informativo | Coda |
| 10 | `/documenti-acquisto-casa` | Media | Thin → allungare | Coda |
| 11 | `/checklist-visita-immobile` | Media | Thin → allungare | Coda |
| 12 | `/tempi-acquisto-casa` | Bassa | Supporto cluster | Coda |
| 13 | `/casa-nuova-o-usata` | Bassa | Supporto cluster | Coda |
| 14 | `/agevolazioni-prima-casa` | Bassa | Già ha link istituzionali; rinforzo copy | Coda |
| 15 | `/quartieri-monza` | Media | Ha mappa; copy da rafforzare | Coda |
| 16 | `/come-funziona` | Media | Trust + processo | Coda |
| 17 | `/faq` | Media | Snippet / PAA | Coda |
| 18 | `/` home prose SEO | Media | Sezioni sotto il form | Coda |
| 19 | `/monza` | Media | Hub geo vendita | Coda |
| 20 | `/brianza` | Media | Hub geo; allineare anche intent acquisto | Coda |
| 21 | `/valutazione/*` comuni (8) | Bassa* | Già contestuali; review a campioni o se thin | Coda |
| 22 | `/valutazione/appartamenti-monza` | Bassa* | Tipologica valutazione | Coda |
| 23 | `/valutazione/ville-brianza` | Bassa* | Tipologica valutazione | Coda |

\*Le leaf valutazione hanno già copy locale: in review, puntare a chiarezza + eventuale ponte verso cluster comprare, non a gonfiare a caso.

## Come procediamo
1. Tu confermi la **prossima** (#2 di default).
2. `/writer:discuss` breve → plan → execute (stesso flusso di Brianza).
3. Ship (commit + push) a fine pezzo.
